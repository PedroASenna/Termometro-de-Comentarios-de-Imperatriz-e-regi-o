...
class NewsScraper:
    ...
    def _scrape_page(self, url, selectors):
        """Raspa uma única página (seja principal ou de categoria) em busca de links de artigos."""
        page_articles = []
        ...
        for link_tag in article_links:
            ...
            if article_url in self.scraped_urls: continue # Evita duplicatas
    ...
    def scrape_all_sites(self):
        logger.info("Iniciando scraping de portais de notícias (versão aprofundada).")
        all_articles = []
        ...
        for site in self.config['sites']:
            ...
            # 1. Raspa a página inicial
            all_articles.extend(self._scrape_page(base_url, selectors))

            # 2. (NOVO) Raspa as páginas de categorias definidas no config
